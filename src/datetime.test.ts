...
describe("dateTimeのテスト", () => {
  ...
  describe("コンストラクタにオプションを指定する", () => {
    ...
    describe("minuteを指定する", () => {
      test.each(allMinutes)("minuteを指定する: %i分", (minute) => {
        const dt = new DateTime({minute})
        expect(dt.getMinutes()).toBe(minute)
      })
    })

    describe("secondを指定する", () => {
      test.each(allSeconds)("secondを指定する: %i秒", (second) => {
        const dt = new DateTime({second})
        expect(dt.getSeconds()).toBe(second)
      })
    })
  ...
  describe("addXXX関数のテスト", () => {
    describe("addMillsのテスト", () => {
      test("ミリ秒を加算しても、元の値+として残っていること", () => {
        const addValue = 300
        const dt = new DateTime({mills: 100})
        const firstMills = dt.getMilliseconds()
        dt.addMills(addValue)
        expect(dt.getMilliseconds()).toBe(firstMills + addValue)
      })
    ...
    describe("addDaysのテスト", () => {
      test("1日足す", () => {
        const addValue = 1
        const dt = new DateTime({year: 2020, month: 5, day: 8, mills: 0})
        dt.addDays(addValue)
      ...
      test("複数日操作する", () => {
        const addValue = 7
        const dt = new DateTime({year: 2020, month: 5, day: 1, mills: 0})
        dt.addDays(addValue)
        expect(dt.getDate()).toBe(1 + addValue)
      })
      describe("1日操作することで、月をまたぐ", () => {
        test("1日足す", () => {
          const addValue = 1
          const dt = new DateTime({year: 2020, month: 4, day: 30, mills: 0})
          dt.addDays(addValue)
        ...
        test("1日減らす(閏年)", () => {
          const addValue = -1
          const dt = new DateTime({year: 2020, month: 3, day: 1, mills: 0})
          dt.addDays(addValue)
    ...
    describe("addHoursのテスト", () => {
      test("1時間足す", () => {
        const addValue = 1
        const dt = new DateTime({year: 2020, month: 4, day: 30, hour: 23, mills: 0})
        dt.addHours(addValue)
        expect(dt.getMonth()).toBe(5)
        expect(dt.getDate()).toBe(1)
      ...
      test("1時間減らす(閏年)", () => {
        const addValue = -1
        const dt = new DateTime({year: 2020, month: 3, day: 1, hour: 0, mills: 999})
        dt.addHours(addValue)
        expect(dt.getMonth()).toBe(2)
        expect(dt.getDate()).toBe(29)
    ...
    describe("addMinutesのテスト", () => {
      test("1分足す", () => {
        const addValue = 1
        const dt = new DateTime({year: 2020, month: 3, day: 1, hour: 0, minute: 59, mills: 999})
        expect(dt.getMinutes()).toBe(59)
        dt.addMinutes(addValue)
        expect(dt.getMinutes()).toBe(0)
        expect(dt.getHours()).toBe(1)
      })
      test("分で減らすことで日付をまたぐ(閏年)", () => {
        const addValue = -(24 * 60)
        const dt = new DateTime({year: 2020, month: 3, day: 1, hour: 0, minute: 0, mills: 999})
        dt.addMinutes(addValue)
        expect(dt.getMinutes()).toBe(0)
        expect(dt.getHours()).toBe(0)
        expect(dt.getMonth()).toBe(2)
    ...
    describe("addSecondsのテスト", () => {
      test("1秒足す", () => {
        const addValue = 1
        const dt = new DateTime({year: 2020, month: 3, day: 1, hour: 0, minute: 59, second: 59, mills: 999})
        expect(dt.getHours()).toBe(0)
        expect(dt.getMinutes()).toBe(59)
        expect(dt.getSeconds()).toBe(59)
        dt.addSeconds(addValue)
        expect(dt.getHours()).toBe(1)
        expect(dt.getMinutes()).toBe(0)
